...
#define debug_dev(s, f, d, ...) printf("%s %s: addr %02x - " s "\n", "BME680", f, d->addr, ## __VA_ARGS__)
#else
#define debug(s, f, ...)
...
#define error_dev(s, f, d, ...) printf("%s %s: addr %02x - " s "\n", "BME680", f, d->addr, ## __VA_ARGS__)
#else
#define error(s, f, ...)
...
bme680_sensor_t* bme680_init_sensor(uint8_t addr)
{
    ...
    if (bme680_i2c_read(dev, BME680_REG_CD1_ADDR, buf+BME680_CDM_OFF1, BME680_REG_CD1_LEN) &&
        bme680_i2c_read(dev, BME680_REG_CD2_ADDR, buf+BME680_CDM_OFF2, BME680_REG_CD2_LEN) &&
        bme680_i2c_read(dev, BME680_REG_CD3_ADDR, buf+BME680_CDM_OFF3, BME680_REG_CD3_LEN))
    {
...
bool bme680_force_measurement (bme680_sensor_t* dev)
{
    ...
    if (dev->meas_started)
    {
        ...
        printf("Measurement is already running.\n");
        return false;
    }
...
bool bme680_is_measuring (bme680_sensor_t* dev)
{
    ...
    if (!bme680_i2c_read(dev, BME680_REG_MEAS_STATUS_0, raw, 2))
    {
...
bool bme680_get_results_fixed (bme680_sensor_t* dev, bme680_values_fixed_t* results)
{
    ...
    if (!bme680_get_raw_data(dev, &raw)){
        ...
        //printf("bme680_get_raw_data failed\n");
        return false;
    }
...
bool bme680_get_results_float (bme680_sensor_t* dev, bme680_values_float_t* results)
{
    ...
    if (!bme680_get_results_fixed (dev, &fixed)) {
        //printf("bme680_get_results_fixed failed\n");
        return false;
    }
    ...
    results->gas_resistance = fixed.gas_resistance / 100.0f;
                                    ...
                                    bme680_oversampling_rate_t osh)
{
    ...
    if (ost_changed || osp_changed)
    {
        ...
        if (!bme680_i2c_read(dev, BME680_REG_CTRL_MEAS, &reg, 1))
            return false;
        ...
        if (!bme680_i2c_write(dev, BME680_REG_CTRL_MEAS, &reg, 1))
            return false;
    }
    ...
    {
        // read the current register value
        if (!bme680_i2c_read(dev, BME680_REG_CTRL_HUM, &reg, 1))
            return false;
        ...
        // write back the new register value
        if (!bme680_i2c_write(dev, BME680_REG_CTRL_HUM, &reg, 1))
            return false;
    }
...
bool bme680_set_filter_size(bme680_sensor_t* dev, bme680_filter_size_t size)
{
    ...
    // read the current register value
    if (!bme680_i2c_read(dev, BME680_REG_CONFIG, &reg, 1))
        return false;
    ...
    // write back the new register value
    if (!bme680_i2c_write(dev, BME680_REG_CONFIG, &reg, 1))
        return false;
                                ...
                                uint16_t temperature, uint16_t duration)
{
    ...
    if (temperature_changed &&
        !bme680_i2c_write(dev, BME680_REG_RES_HEAT_BASE+profile, &heat_res, 1))
        return false;

    if (duration_changed &&
        !bme680_i2c_write(dev, BME680_REG_GAS_WAIT_BASE+profile, &heat_dur, 1))
        return false;
...
bool bme680_use_heater_profile (bme680_sensor_t* dev, int8_t profile)
{
    ...
    if (!bme680_i2c_write(dev, BME680_REG_CTRL_GAS_1, &reg, 1))
        return false;
...
bool bme680_set_ambient_temperature (bme680_sensor_t* dev, int16_t ambient)
{
    ...
    if (!bme680_i2c_write(dev, BME680_REG_RES_HEAT_BASE, data, 10))
        return false;
...
static bool bme680_is_available (bme680_sensor_t* dev)
{
    ...
    if (!bme680_i2c_read (dev, BME680_REG_ID, &chip_id, 1))
        return false;
...
static bool bme680_reset (bme680_sensor_t* dev)
{
    ...
    // send reset command
    if (!bme680_i2c_write(dev, BME680_REG_RESET, &reg, 1))
        return false;
    ...
    if (!bme680_i2c_read(dev, BME680_REG_STATUS, &reg, 1))
    {
...
static bool bme680_get_raw_data(bme680_sensor_t *dev, bme680_raw_data_t* raw_data)
{
    ...
    if (!bme680_i2c_read(dev, BME680_REG_RAW_DATA_0, raw, BME680_REG_RAW_DATA_LEN))
    {
...
static bool bme680_i2c_read(bme680_sensor_t* dev, uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    i2c_master_write_byte(cmd, (BME680_I2C_ADDRESS_1 << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, reg, true);
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (BME680_I2C_ADDRESS_1 << 1) | I2C_MASTER_READ, true);
    if (len > 1)
    {
    ...
    {
        // debug_dev ("I2C read error: %s", esp_err_to_name(ret));
...
static bool bme680_i2c_write(bme680_sensor_t* dev, uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    {
        //debug_dev ("I2C write error: %s", esp_err_to_name(ret));
        return false;
    }
}
